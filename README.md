...
## Demo — WP Notes Plugin (Built on WP Pillar + Vue 3)

> A full-stack Notes Manager plugin built to validate the WP Pillar framework end-to-end with Vue 3 + Vite, Eloquent ORM, and REST API.
...
**Install via WordPress dashboard:** Plugins → Add New → Upload Plugin → select `wp-notes.zip`
...
2. **`illuminate/pagination` must be listed explicitly** in `composer.json`:
...
3. **Add `Schema::hasTable()` guard in every migration** to prevent errors on plugin reactivation:

```php
public function up(): void
{
    if (ORM::schema()->hasTable('your_table')) {
        return;
    }
    // create table...
}
